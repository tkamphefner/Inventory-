...
import { ShoppingCart, ArrowRight, Plus, Filter, Download } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const fetchSessions = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/sessions?sessionType=checkout&status=active', {
        headers: {
  ...
  const actions = [
    {
      ...
      onClick: (session) => router.push(`/checkout/${session.id}`),
      color: 'blue'
    }
  ];
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-6">
            <h2 className="text-lg font-medium text-gray-900">Active Checkout Sessions</h2>
            <p className="text-sm text-gray-500">
              View and manage ongoing checkout sessions or create a new one.
            </p>
          </div>
          ...
          <DataTable
            ...
            data={sessions}
            actions={actions}
            onRowClick={(session) => router.push(`/checkout/${session.id}`)}
            onAdd={() => {
              fetchLocations();
      ...
      {showNewSessionModal && (
        <Modal
          ...
          title="Start New Checkout Session"
          size="md"
          footer={
            <>
              <Button
          ...
          <form id="new-session-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Add any notes about this checkout session"
